...
このツールは、Strava の記録を 1 年ごとにグラフ化して、自分の成長を確認できる場所です。

## こだわったのは、究極のプライバシー

普通のアプリは、あなたのデータを運営者のサーバーに送って処理します。でも、この
ツールは違います。

あなた自身の場所に「データの通り道 (Worker)」を作ってもらうことで、開発者ですら、
あなたがどこを走ったか知ることはできません。

データの流れ: `Strava の倉庫 ↔ あなた専用の通訳 ↔ この画面`

- **走った記録はどこ?** あなたのブラウザの中だけ。サーバーには送られません。
- **「通訳」って何?** Strava から記録を借りてくる窓口役 (= Cloudflare Worker)。
  あなた自身で立てるので、開発者の手元を通りません。
- **コードは見える?** 通訳が動かすコードは [worker/index.js](./worker/index.js)
  (約 90 行)。中継しかしません。

設定は最初の一回だけ。あなたの大切なデータを、誰の手にも渡さないための「鍵」を
自分で持つ。そんな、誠実な分析ツールを目指しました。

## 安心のための 3 つの準備

1. **鍵を借りる**: Strava の「貸し出し窓口」に申し込んで Client ID を控える
2. **通訳を雇う**: Cloudflare に「あなた専用の通訳さん」を立てる
3. **画面に取り付ける**: 鍵と通訳の住所をこの画面に教える
