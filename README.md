...
**究極のプライバシーを追求した、Strava Fitness & Freshness 年別分析ツール**

Strava 公式の Fitness & Freshness チャートは便利ですが、特定の年度をサッと
振り返ったり、過去の自分と比較したりするには少し不便です。本ツールは、
Strava API を使って「年度ごとのコンディション推移」をワンクリックで可視化する
SPA (シングルページアプリケーション) です。

## 最大の特徴: Zero-Data-Access アーキテクチャ

多くのサードパーティアプリと異なり、本ツールには中央サーバーがありません。

- **データの非保持**: 取得したアクティビティはあなたのブラウザの
  `localStorage` にのみ保存され、開発者にもクラウド事業者の DB にも渡りません。
- **自分専用の中継機 (Worker)**: セットアップ時に、あなた自身の Cloudflare
  アカウントに小さな中継プログラムを設置していただきます。Strava との通信に
  必要な「秘密鍵 (Client Secret)」を開発者に預けることなく、あなただけが
  管理する形で安全に Strava と通信します。
- **データの流れ**: `Strava ↔ あなたの Worker ↔ あなたのブラウザ`。
  この経路に開発者は一切介入できません。

> 「自分のデータは、自分だけの管理下に置きたい。でも、年度別の CTL / TSB 推移
> をじっくり眺めたい。」
> ── そんなこだわりを持つサイクリスト・ランナーのための、セルフホスト型分析ツールです。
