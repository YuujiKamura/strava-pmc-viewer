...
Strava 公式 UI では難しい「年度ごとのコンディション比較」を、ブラウザ上だけで
完結させるツールです。

## 何ができるか

- 年度ボタンを 1 つ押すと、その年の Fitness (CTL) / Fatigue (ATL) / Form (TSB)
  チャートが出る
- 任意区間にドラッグ + ホイールでズーム、チャート上の任意の日をクリックすると
  前後 ±3 日のアクティビティ一覧 + Strava へのリンク

## なぜセルフホスト (Worker) が必要か

Strava API を利用するには Client Secret (秘密鍵) が必要です。一般的な Web
サービスのように運営者サーバーに保存すると、運営側があなたのデータに
アクセスできてしまうリスクが生じます。

本ツールは中央サーバーを持ちません。代わりに、ユーザー自身に「自分専用の
中継機 (Cloudflare Worker)」をデプロイしていただきます。

- 開発者はあなたのデータを見ることができない
- 秘密鍵はあなたの管理下にある Worker 内にのみ存在する
- データは Strava とあなたのブラウザの間だけで完結する

API キーを他人に預けるのは抵抗があるが、詳細な分析はしたい人のためのツール
です。セットアップには Cloudflare Workers のデプロイなど手間が要りますが、
一度終われば、その分析環境はあなただけの管理下にあります。
