...
function dailyNpFromActivities(activities, mode) {
  ...
  // mode: "real" (device_watts==true のみ)、 "est" (実測 ride が 1 本も無い年のみ推定)、
  // それ以外 (or 未指定) は実測 / 推定 / 不明 を全部合算 = "all"。
  // est は per-year priority: その年に実測 ride が 1 本でもあれば est は全期間出さない
  // (= 「実測がある年は推定線は丸ごとノイズ」 という user の判断、 2026-06-06)。
  const anyRealInYear = activities.some(a =>
    a.device_watts === true && (a.sport_type || a.type || "").endsWith("Ride")
  );
  if (mode === "est" && anyRealInYear) return new Map();
  const agg = new Map();
